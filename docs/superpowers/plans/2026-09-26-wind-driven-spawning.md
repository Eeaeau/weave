...
**Interfaces:** `WindEvent3D.start_round(round_number: int) -> void`, `is_active: bool`, `event_started(round_number: int)`, `item_contact(item: Collectible3D, side: int, local_point: Vector2, world_point: Vector3)`, and `event_finished(round_number: int)`. The `WindEvent` scene contains a `WebPlane` marker, left/right `WindLane3D` markers, and a flight container. The local point maps the plane's X/Z axes to `Vector2(x, z)`. `WindEvent3D` consumes Task 1 selection and Task 2 flights. `wind_preview.tscn` instantiates the canopy map, calls `start_round(1)` at scene start, and logs contact and finish signals; regular `web_match.tscn` does not auto-start it.
