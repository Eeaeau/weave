...
src/
  ...
  scripts/                      Shared scripts if needed later
...
Keep a scene, its script, and assets used only by that scene in one feature folder. Use top-level `assets/` for files shared by several features. Empty placeholder folders contain `.gitkeep` so they remain visible in Git.
